...
import { GuardPaymentFormValues, guardPaymentSchema } from "@/schemas";
import { useGetAllGuardsQuery } from "@/store/apis/guardsApi";
import { useGetGuardTimeSheetSummaryQuery } from "@/store/apis/schedulingAPI";
import {
  Select,
  SelectContent,
  SelectItem,
...
const GuardPaymentForm = ({
  ...
  const overtimeHourlyRate = watch("overtimeHourlyRate");
  ...
  useEffect(() => {
    ...
    const shiftIds = summaryData.data.details.map(
      (s: GuardTimeSheetDetails) => s.shiftId,
    );

    setValue("staticIds", shiftIds || []);

    setValue("regularHours", summary.totalRegularHours || 0);

    setValue("overtimeHours", summary.totalOvertimeHours || 0);
  }, [summaryData, setValue]);

  useEffect(() => {
    const calculatedBasePay = hourlyRate * regularHours;

    const calculatedOvertimePay = overtimeHourlyRate * overtimeHours;

    const calculatedTotalHours = regularHours + overtimeHours;
    ...
    setValue("basePay", Number(calculatedBasePay.toFixed(2)));

    setValue("overtimePay", Number(calculatedOvertimePay.toFixed(2)));

    setValue("totalHours", Number(calculatedTotalHours.toFixed(2)));

    setValue("totalPay", Number(calculatedTotalPay.toFixed(2)));
  }, [
    hourlyRate,
    overtimeHourlyRate,
    regularHours,
          ...
          <Controller
            ...
            render={({ field }) => (
              <Field>
                ...
                <Input type="date" {...field} min={fromDate || undefined} />
              </Field>
            )}
          />
        </div>
      ...
      <FieldGroup>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field>
                <FieldLabel>Overtime Hourly Rate</FieldLabel>
      ...
      <FieldGroup>
        ...
        <div className="grid gap-2 grid-cols-2">
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field>
                <FieldLabel>Tax Deduction</FieldLabel>
          ...
          <CardContent className="space-y-2 p-4">
            <div className="flex items-center justify-between description">
              <p className="text-muted-foreground">
                Base Pay ({regularHours}h × ${hourlyRate})
              </p>

              <p className="font-medium">{formatCurrency(basePay)}</p>
            </div>

            <div className="flex items-center justify-between description">
              <p className="text-muted-foreground">
                Overtime Pay ({overtimeHours}h × ${overtimeHourlyRate})
              </p>

              <p className="font-medium">{formatCurrency(overtimePay)}</p>
            </div>
            ...
            <div className="flex items-center justify-between description">
              ...
              <p className="font-medium">
                {formatCurrency(basePay + overtimePay)}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between description">
              <p className="text-muted-foreground">Other Deduction</p>
