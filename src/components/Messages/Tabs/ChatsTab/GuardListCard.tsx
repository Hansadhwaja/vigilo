...
const GuardListCard = ({
  ...
  const knownCid = conversationByUserId[guard.id];

  const isActive =
    selectedGuard?.id === guard.id ||
    (!!activeConversationId && knownCid === activeConversationId);

  const online = presenceMap.get(guard.id)?.isOnline ?? false;

  const isOpening = openingUserId === guard.id;

  return (
    <button
      key={guard.id}
      type="button"
      onClick={() => openGuardChat(guard)}
      className={cn(
        "group relative flex w-full items-center gap-3 overflow-hidden rounded-2xl border p-3 text-left transition-all duration-200",
        isActive
    ...
    >
      ...
      <div className="relative shrink-0">
        <UserAvatar src="" name={guard.name} />
