...
import { Download, FileText, User, Calendar, DollarSign } from "lucide-react";
  ...
  const columns: Column<InvoiceType & RowWithId>[] = [
      ...
      render: (row) => (
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <p className="font-semibold text-slate-800">{row.invoiceNumber}</p>
          </div>
...

    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          ...
          <p className="font-medium text-slate-700">{row.clientName}</p>
        </div>
      ),
    },
...

    {
      ...
      render: (row) => (
        ...
        >
          {loadingId === row.id ? <Loader /> : <Download className="h-4 w-4" />}
          PDF
        </Button>
      ),
    },
