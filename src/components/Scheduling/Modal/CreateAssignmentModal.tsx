...
import { useCreateScheduleMutation } from "@/store/apis/schedulingAPI";

interface CreateAssignmentModalProps {
  title?: string;
}

const CreateAssignmentModal = ({ title }: CreateAssignmentModalProps) => {
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border border-slate-200 p-0 shadow-xl sm:max-w-3xl">
        ...
        <DialogHeader className="border-b border-slate-100 bg-slate-50 px-6 py-5 text-left">
          ...
          <DialogDescription className="mt-1 text-sm text-slate-500">
            Assign guards to a schedule and manage shift timings.
          </DialogDescription>
        </DialogHeader>
