...
export const invoiceSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Add at least one order, service, or alarm to create an invoice.",
      path: ["orders"],
    },
  );
...
export const servicePricingSchema = z.object({
  ...
  dailyPrice: z.string().refine((val) => val === undefined || Number(val) > 0, {
...
export const assignmentSchema = z
  .object({
    ...
    guardIds: z.array(z.string()).min(1, "Select at least one guard"),
...
export const orderSchema = z
  .object({
    ...
    siteServiceLat: z.number().refine((val) => val !== 0, {
      message: "Latitude is required",
    }),

    siteServiceLng: z.number().refine((val) => val !== 0, {
...
export const timeSheetSchema = z
  ...
  .refine((data) => data.shiftEndTime > data.shiftStartTime, {
    message: "End time must be after start time",
    path: ["shiftEndTime"],
  });
...
export const guardPaymentSchema = z
  .object({
    ...
    taxDeduction: z.number().min(0, "Tax deduction cannot be negative"),
...
export type GuardPaymentFormValues = z.infer<typeof guardPaymentSchema>;

//Patrolling

export const checkpointSchema = z.object({
  name: z.string().min(3, "Checkpoint name must be at least 3 characters"),
...
export const subSiteSchema = z.object({
  name: z.string().min(3, "Sub-site name must be at least 3 characters"),
