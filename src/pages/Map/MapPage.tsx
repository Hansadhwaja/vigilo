...
export default function MapPage({ onSelectGuard }: MapPageProps) {
  ...
  const getVehicleColor = (vehicle: any) => {
    switch (vehicle.status) {
      case "Active":
        return vehicle.speed > 0 ? "#22c55e" : "#3b82f6";
      case "Maintenance":
        return "#f59e0b";
      default:
        return "#6b7280";
  ...
  return (
    <div className="space-y-6 overflow-y-auto min-w-0 min-h-0 h-full no-scrollbar">
      {/* API Status Bar */}
      <Card
        className={`${apiConnected ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
      >
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${apiConnected ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
              ></div>
              <div className="flex items-center gap-2">
                <Satellite
                  className={`h-4 w-4 ${apiConnected ? "text-green-600" : "text-red-600"}`}
                />
                <span
                  className={`text-xl font-medium ${apiConnected ? "text-green-800" : "text-red-800"}`}
                >
            ...
            <div className="flex items-center gap-4 text-xl">
              <div className="flex items-center gap-1">
                <RefreshCw
                  className={`h-4 w-4 ${apiConnected ? "text-green-600" : "text-red-600"} ${apiConnected ? "animate-spin" : ""}`}
                />
                <span
                  className={apiConnected ? "text-green-700" : "text-red-700"}
                >
          ...
          <CardContent>
            <div className="relative w-full" style={{ height: 480 }}>
              <svg
                ...
                className="w-full h-full rounded-lg bg-linear-to-br from-slate-100 to-slate-200 border"
              >
                {/* Background grid */}
                <defs>
                  <pattern
                ...
                {/* Sites / geofences */}
                {(mapLayer === "all" || mapLayer === "guards") &&
                  trackingFilters.geofences &&
                  sampleSites.map((s, i) => (
                    <g key={s.id}>
                      <rect
                        x={s.bounds.x}
                ...
                {(mapLayer === "all" ||
                  ...
                  mapLayer === "vehicles") &&
                  showTrails &&
                  trackingFilters.routes &&
                  sampleVehicles
                    .filter((v) => v.route.length > 1)
                    .map((vehicle) => {
                      const points = vehicle.route.map((point) =>
                        mapConfig.project(point.lat, point.lng),
                      );
                      ...
                      return (
                        <g key={`trail-${vehicle.id}`}>
                          <path
                            d={pathData}
                            stroke={getVehicleColor(vehicle)}
                            strokeWidth="3"
                            fill="none"
                            strokeDasharray="5,5"
                            opacity={0.6}
                ...
                {/* Vehicle markers */}
                {(mapLayer === "all" || mapLayer === "vehicles") &&
                  trackingFilters.vehicles &&
                  sampleVehicles.map((vehicle) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isMoving && (
                          <polygon
                            points={`${p.x},${p.y - 12} ${p.x - 5},${p.y - 6} ${p.x + 5},${p.y - 6}`}
                            fill="white"
                            transform={`rotate(${vehicle.heading} ${p.x} ${p.y})`}
                          />
                        )}
        ...
        <div className="space-y-4">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setTrackingFilters((prev) => ({ ...prev, guards: checked }))
                  }
                />
              </div>
              <div className="flex items-center justify-between">
              ...
              {sampleVehicles.map((vehicle) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-3 h-3 rounded-full ${vehicle.speed > 0 ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
                      ></div>
                    ...
                    <Badge
                      className={`${
                        getVehicleColor(vehicle) === "#22c55e"
                          ? "bg-green-100 text-green-800"
                          : getVehicleColor(vehicle) === "#f59e0b"
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Vehicle tracking system connection
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
      ...
      >
        <SheetContent className="w-[500px]">
          <SheetHeader>
            <SheetTitle>
              Vehicle Details: {selectedVehicle?.callsign}
            </SheetTitle>
            <SheetDescription>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-xl">
                  <div>
                    ...
                    <Badge
                      className={`ml-2 ${selectedVehicle.status === "Active" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}
                    >
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Progress
                      value={(selectedVehicle.engineTemp / 120) * 100}
                      className="h-2"
                    />
                  </div>
                </div>
