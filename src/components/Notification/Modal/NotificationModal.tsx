...
const NotificationModal = () => {
  ...
  const unreadCount = notificationsResponse?.counts?.unread ?? 0;
  ...
  const handleDeleteAllNotifications = async () => {
    try {
      ...
      toast.error(
        err?.data?.message || err?.error || "Failed to delete notifications",
      );
    }
  };
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="w-100 rounded-2xl border border-slate-200 p-0 shadow-xl"
      >
        {/* HEADER */}
          ...
          {notifications.length > 0 ? (
            <div className="space-y-2">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`group rounded-xl border p-3 transition-all hover:bg-slate-50 ${
                    notification.isRead
