...
export const useTypingIndicator = ({
  ...
  const selectedGuardRef = useRef<Guard | null>(null);

  useEffect(() => {
    selectedGuardRef.current = selectedGuard;
  }, [selectedGuard]);
  ...
  useEffect(() => {
    ...
    const handleTyping = (payload: TypingEvent) => {
      ...
      const currentGuardId = selectedGuardRef.current?.id;

      if (currentGuardId && String(payload.userId) !== currentGuardId) return;

      setIsOtherTyping(true);
    };

    const handleStopTyping = (payload: TypingEvent) => {
      if (payload?.conversationId !== activeConversationId) return;
